...
function SkillDetailDialog({ skill, isOpen, onClose, onToggle, onUninstall, onOpenFolder }: SkillDetailDialogProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        className="w-full sm:max-w-[450px] p-0 flex flex-col border-l border-primary/10 bg-[hsl(var(--surface-muted))] dark:bg-card shadow-[0_0_40px_rgba(0,0,0,0.2)]"
        side="right"
      >
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto px-8 py-10">
          <div className="flex flex-col items-center mb-8">
            <div className="w-16 h-16 flex items-center justify-center rounded-full bg-white dark:bg-accent border border-primary/10 shrink-0 mb-4 relative shadow-sm">
              <span className="text-3xl">{skill.icon || '🔧'}</span>
              {skill.isCore && (
                <div className="absolute -bottom-1 -right-1 bg-[hsl(var(--surface-muted))] dark:bg-card rounded-full p-1 shadow-sm border border-primary/10">
          ...
          <div className="space-y-7 px-1">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  readOnly
                  className="h-[38px] font-mono text-[12px] bg-[hsl(var(--surface))] dark:bg-muted border-primary/10 rounded-xl text-foreground/70"
                />
                <Button
                  variant="outline"
                  size="icon"
                  className="h-[38px] w-[38px] border-primary/10"
            ...
            {!skill.isCore && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="password"
                  className="h-[44px] font-mono text-[13px] bg-[hsl(var(--surface))] dark:bg-muted border-primary/10 rounded-xl focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:border-blue-500 shadow-sm transition-all text-foreground placeholder:text-foreground/40"
                />
            ...
            {!skill.isCore && (
              <div className="space-y-3">
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    <h3 className="text-[13px] font-bold text-foreground/80">
                      {t('detail.envVars')}
                      {envVars.length > 0 && (
                        <Badge variant="secondary" className="ml-2 px-1.5 py-0 text-[10px] h-5 bg-primary/10 text-foreground">
                          {envVars.length}
                        </Badge>
                      )}
                    </h3>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 text-[12px] font-semibold text-foreground/80 gap-1.5 px-2.5 hover:bg-primary/5"
                ...
                <div className="space-y-2">
                  {envVars.length === 0 && (
                    <div className="text-[13px] text-foreground/50 font-medium italic flex items-center bg-[hsl(var(--surface))] dark:bg-muted border border-primary/10 rounded-xl px-4 py-3 shadow-sm">
                  ...
                  {envVars.map((env, index) => (
                    <div className="flex items-center gap-3" key={index}>
                      <Input
                        ...
                        className="flex-1 h-[40px] font-mono text-[13px] bg-[hsl(var(--surface))] dark:bg-muted border-primary/10 rounded-xl focus-visible:ring-2 focus-visible:ring-blue-500/50 shadow-sm text-foreground"
            ...
            {skill.slug && !skill.isBundled && !skill.isCore && (
              <div className="flex gap-2 justify-center pt-8">
                <Button variant="outline" size="sm" className="h-[28px] text-[11px] font-medium px-3 gap-1.5 rounded-full border-primary/10 bg-transparent hover:bg-primary/5 shadow-none text-foreground/70" onClick={handleOpenClawhub}>
                  <Globe className="h-[12px] w-[12px]" />
                  ClawHub
                </Button>
                <Button variant="outline" size="sm" className="h-[28px] text-[11px] font-medium px-3 gap-1.5 rounded-full border-primary/10 bg-transparent hover:bg-primary/5 shadow-none text-foreground/70" onClick={handleOpenEditor}>
          ...
          <div className="pt-8 pb-4 flex items-center justify-center gap-4 w-full px-2 max-w-[340px] mx-auto">
            ...
            {!skill.isCore && (
              <Button
                variant="outline"
                className="flex-1 h-[42px] text-[13px] rounded-full font-semibold shadow-sm bg-transparent border-primary/10 hover:bg-primary/5 transition-colors text-foreground/80 hover:text-foreground"
                onClick={() => {
  ...
  return (
    <div className="flex flex-col -m-6 dark:bg-background h-[calc(100vh-2.5rem)] overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full p-10 pt-16">
          ...
          <div className="flex items-center gap-3 md:mt-2">
            {hasInstalledSkills && (
              <button
                ...
                className="hover:bg-primary/5 transition-colors shrink-0 text-[13px] font-medium px-4 h-8 rounded-full border border-primary/10 flex items-center justify-center text-foreground/80 hover:text-foreground"
              >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-primary/10 pb-4 mb-4 shrink-0 gap-4">
          <div className="flex items-center flex-wrap gap-4 text-[14px]">
            <div className="relative group flex items-center bg-primary/5 rounded-full px-3 py-1.5 focus-within:bg-black/10 transition-colors border border-transparent focus-within:border-black/10 dark:focus-within:border-white/10 mr-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              className="h-8 text-[13px] font-medium rounded-md px-3 border-primary/10 bg-transparent hover:bg-primary/5 shadow-none"
            >
            ...
            <Button
              ...
              className="h-8 w-8 ml-1 rounded-md border-primary/10 bg-transparent hover:bg-primary/5 shadow-none text-muted-foreground hover:text-foreground"
              title={t('refresh')}
            >
          ...
          <div className="flex flex-col gap-1">
            {filteredSkills.length === 0 ? (
              ...
              filteredSkills.map((skill) => (
                <div
                  key={skill.id}
                  className="group flex flex-row items-center justify-between py-3.5 px-3 rounded-xl hover:bg-primary/5 transition-colors cursor-pointer border-b border-primary/10 last:border-0"
                  onClick={() => setSelectedSkill(skill)}
                >
                  <div className="flex items-start gap-4 flex-1 overflow-hidden pr-4">
                    <div className="h-10 w-10 shrink-0 flex items-center justify-center text-2xl bg-primary/5 border border-primary/10 rounded-xl overflow-hidden">
                      {skill.icon || '🧩'}
                    </div>
                    <div className="flex flex-col overflow-hidden">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {skill.slug && skill.slug !== skill.name ? (
                          <span className="text-[11px] font-mono px-1.5 py-0.5 rounded border border-primary/10 text-muted-foreground">
                            {skill.slug}
                          </span>
                        ) : null}
                      </div>
                      ...
                      <div className="mt-1 flex items-center gap-2 text-[11px] text-foreground/55">
                        <Badge variant="secondary" className="px-1.5 py-0 h-5 text-[10px] font-medium bg-primary/10 border-0 shadow-none">
                          {resolveSkillSourceLabel(skill, t)}
                        </Badge>
      ...
      <Sheet open={installSheetOpen} onOpenChange={setInstallSheetOpen}>
        <SheetContent
          className="w-full sm:max-w-[560px] p-0 flex flex-col border-l border-primary/10 bg-[hsl(var(--surface-muted))] dark:bg-card shadow-[0_0_40px_rgba(0,0,0,0.2)]"
          side="right"
        >
          <div className="px-7 py-6 border-b border-primary/10">
            ...
            <div className="mt-4 flex flex-col md:flex-row gap-2">
              <div className="relative flex items-center bg-primary/5 rounded-xl px-3 py-2 border border-primary/10 flex-1">
                <Search className="h-4 w-4 shrink-0 text-muted-foreground" />
                <Input
              ...
              <Button
                variant="outline"
                disabled
                className="h-10 rounded-xl border-primary/10 bg-transparent text-muted-foreground"
              >
              ...
              <div className="flex flex-col gap-1">
                {searchResults.map((skill) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-4 flex-1 overflow-hidden pr-4">
                        <div className="h-10 w-10 shrink-0 flex items-center justify-center text-xl bg-primary/5 border border-primary/10 rounded-xl overflow-hidden">
                          📦
                        </div>
                        <div className="flex flex-col overflow-hidden">
                          <div className="flex items-center gap-2 mb-1">
